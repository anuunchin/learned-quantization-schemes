...
class ScaleTrackingCallback(tf.keras.callbacks.Callback):
    ...
    def plot_scale_values(self, layer_name, folder_name):
        ...
        for i in range(len(self.scale_values_per_epoch_w[0])):
            ...
            plt.plot(
                range(1, len(self.scale_values_per_epoch_w) + 1),
                scale_trajectory,
                linestyle="-",
                marker="o",
            )
        ...
        plt.savefig(
            os.path.join(
                folder_name, "Scale Values of w per Epoch - " + layer_name + ".png"
            )
        )
        ...
        for i in range(len(self.scale_values_per_epoch_b[0])):
            ...
            plt.plot(
                range(1, len(self.scale_values_per_epoch_b) + 1),
                scale_trajectory,
                linestyle="-",
                marker="o",
            )
        ...
        plt.savefig(
            os.path.join(
                folder_name, "Scale Values of b per Epoch - " + layer_name + ".png"
            )
        )
...
class LossTrackingCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        scale_loss = calculate_average_loss_epoch(
            scale_loss_file, start_line=start_line, end_line=end_line
        )
        total_loss = calculate_average_loss_epoch(
            total_loss_file, start_line=start_line, end_line=end_line
        )
    ...
    def plot_loss(self, folder_name):
        ...
        plt.fill_between(
            epochs, 0, self.epoch_scc_loss, color="grey", alpha=0.5, label="Total Loss"
        )
        ...
        plt.ylabel("Loss")
        plt.title(
            f"Loss structure per epoch with penalty rate {self.penalty_rate} and loss function {self.loss_function_name}"
        )
        ...
        plt.savefig(
            os.path.join(
                folder_name,
                f"Loss structure per epoch with penalty rate {self.penalty_rate} and loss function {self.loss_function_name}.png",
            )
        )
...
class LossTrackingCallbackNew(tf.keras.callbacks.Callback):
    ...
    def on_train_batch_end(self, batch, logs=None):
        ...
        batch_line = (
            self.current_epoch * (1875 + 313)
            + (int(batch / self.interval) * 313)
            + batch
            + 1
        )

        batch_total_loss = calculate_average_loss_epoch(
            total_loss_file, start_line=batch_line, end_line=batch_line
        )
        batch_scale_loss = calculate_average_loss_epoch(
            scale_loss_file, start_line=batch_line, end_line=batch_line
        )
...
"""
        ...
        if self.batch_count % self.interval == 0:
            ...
            with open(f"logs/validation", "a") as file:
    ...
    def plot_loss_per_batch(self, folder_name, n=20):
        ...
        plt.fill_between(
            batches,
            0,
            self.batch_scc_loss[n:],
            color="grey",
            alpha=0.5,
            label="Total loss per batch",
        )
        ...
        plt.fill_between(
            batches,
            0,
            self.batch_scale_penalty_loss[n:],
            color="none",
            edgecolor="red",
            hatch="///",
            linewidth=0,
        ...
        plt.scatter(
            ...
            zorder=3,
        )  # Use zorder to make sure it's on top
        plt.plot(
            validation_batches, validation_losses, "b--", linewidth=2
        )  # Optionally connect the points with a dashed line

        plt.xlabel("Batches", labelpad=40)
        plt.ylabel("Loss")
        plt.title(
            f"Loss structure per batch with penalty rate {self.penalty_rate} and loss function {self.loss_function_name}"
        )
        ...
        plt.xticks(
            xticks, labels=[""] * len(xticks)
        )  # Set the x-ticks but without labels
        ...
        ax.set_xticks(
            [tick for tick in xticks if tick not in epoch_ticks]
        )  # Reset ticks for batch numbers only

        for i, tick in enumerate(xticks):
            if tick in epoch_ticks:
                ax.text(
                    tick,
                    ax.get_ylim()[0] - 0.02 * ax.get_ylim()[1],
                    xtick_labels[i],
                    ha="center",
                    va="top",
                    rotation=0,
        ...
        plt.savefig(
            os.path.join(
                folder_name,
                f"Loss structure per batch with penalty rate {self.penalty_rate} and loss function {self.loss_function_name}.png",
            )
        )
