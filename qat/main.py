...
from custom_callbacks import (
    AccuracyTrackingCallBack,
    LossTrackingCallback,
    ScaleTrackingCallback,
)
...
from utils import count_unique_values, print_model_structure
...
loss_function = SCCEMinMaxBin(
    layers=[quantized_model.get_layer(index=2), quantized_model.get_layer(index=3)],
    penalty_rate=0.1,
...
scale_tracking_callback_first_dense_layer = ScaleTrackingCallback(
    quantized_model.get_layer(index=2)
)
scale_tracking_callback_second_dense_layer = ScaleTrackingCallback(
...
# Plot
scale_tracking_callback_first_dense_layer.plot_scale_values(
    layer_name="Quantized Dense Layer 1"
)
scale_tracking_callback_second_dense_layer.plot_scale_values(
    layer_name="Quantized Dense Layer 2"
)
