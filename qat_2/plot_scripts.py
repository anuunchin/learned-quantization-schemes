...
def plot_scale_values(layer, log_dir):
    ...
    log_file_path_w = (
        f"{log_dir}/{layer.__class__.__name__}_scale_w_{layer.scale_w.shape}.log"
    )
    ...
    plt.title(
        f"scale_w of shape {layer.scale_w.shape} of layer {layer.__class__.__name__}"
    )
    ...
    plt.savefig(
        os.path.join(
            plots_dir,
            f"scale_w_of_shape_{layer.scale_w.shape}_of_layer_{layer.__class__.__name__}.png",
        )
    )
    ...
    plt.ylabel("Scale Values")
    plt.title(
        f"scale_b of shape {layer.scale_b.shape} of layer {layer.__class__.__name__}"
    )
    ...
    plt.savefig(
        os.path.join(
            plots_dir,
            f"scale_b_of_shape_{layer.scale_b.shape}_of_layer_{layer.__class__.__name__}_with_scale_w_of_shape_{layer.scale_w.shape}.png",
        )
    )
...
def validation_losses(log_dir):
    ...
    with open(val_loss_file) as file:
        for line in file:
            line = line.strip()
...
def get_training_loss(log_dir, type):
    ...
    with open(total_loss_file) as file:
        for line in file:
...
def plot_training_validation_loss(log_dir, interval, epochs, x_train_size, batch_size):
    ...
    epoch_ticks = list(
        range(int(batches_per_epoch / 2), total_batches + 1, batches_per_epoch)
    )
    ...
    plt.plot(
        val_loss_range, val_loss, linestyle="-", color="red", label="Validation Loss"
    )
    plt.xlabel("Batch")
    plt.ylabel("Loss")
    ...
    description_text = (
        "Full validation is conducted at specified batch intervals.\n"
...
def plot_loss(log_dir, x_train_size, batch_size, penalty_rate):
    ...
    plt.fill_between(
        epochs, 0, scale_avg, color="none", edgecolor="red", hatch="///", linewidth=0
    )
    ...
    plt.savefig(
        os.path.join(
            plots_dir, f"Loss structure per epoch with penalty rate {penalty_rate}.png"
        )
    )
...
def plot_accuracy(log_dir):
    ...
    if os.path.exists(vanilla_accuracy_log_path):
        with open(vanilla_accuracy_log_path) as f:
            for line in f:
    ...
    if accuracies:
        plt.plot(
            ...
            linestyle="-",
            color="red",
            label="Quantized Model Accuracy",
        )
    ...
    if vanilla_accuracies:
        plt.plot(
            ...
            linestyle="-",
            color="red",
            label="Vanilla Model Accuracy",
        )

    if accuracies or vanilla_accuracies:
        plt.xticks(
            range(1, len(accuracies) + 1)
        )  # Set x-axis ticks to match the number of epochs

        # Convert y-axis to percentage
        plt.gca().yaxis.set_major_formatter(
            mtick.PercentFormatter(xmax=1.0)
        )  # Assuming accuracy is between 0 and 1
        plt.xlabel("Epochs")
        plt.ylabel("Accuracy (%)")
...
def plot_pr_accuracy(log_dirs, penalty_rates):
    ...
    def plot(accuracies, penalty_rates):
        ...
        for epoch, acc_values in accuracies.items():
            plt.plot(
                acc_values, penalty_indices, marker="o", label=f"Epoch {epoch}"
            )  # Plot with indices
...
def plot_pareto(log_dirs, penalty_rates, exclude=0):
    ...
    total_losses[: len(penalty_rates) - exclude]
...
def plot_unique_values_accuracy(log_dirs, penalty_rates):
    ...
    for log_dir, pr in zip(log_dirs, penalty_rates):
        ...
        unique_values = 0
        with open(unique_values_los_path) as f:
            for index, line in enumerate(f):
    ...
    for log_dir in log_dirs:
        ...
        if accuracies:
            plt.plot(
                ...
                marker="o",
                linestyle="-",
                label=f"Quantized Model Accuracy ({os.path.basename(log_dir)})",
            )

        if vanilla_accuracies:
            plt.plot(
                range(1, len(vanilla_accuracies) + 1),
                vanilla_accuracies,
                marker="o",
                linestyle="--",
                label=f"Vanilla Model Accuracy ({os.path.basename(log_dir)})",
            )

    if any(
        os.path.exists(os.path.join(log_dir, "accuracy.log"))
        or os.path.exists(os.path.join(log_dir, "vanilla_accuracy.log"))
        for log_dir in log_dirs
    ):
        plt.xticks(
            range(1, len(accuracies) + 1)
                ...
                for line in f:
                    if line.startswith("Unique values in quantized w:"):
                        try:
                            ...
                            print(
                                f"Warning: Could not parse value in file {file_name} on line: {line.strip()}"
                            )
            if quantized_w_sum > 0:
                label = f"{parent_dir}"  # Use the parent directory as part of the label
                print(
                    f"File: {label}, Sum of Unique Quantized W Values: {quantized_w_sum}"
                )
    ...
    if quantized_w_sums:
        ...
        for bar in bars:
            plt.text(
                ...
                va="center",
                ha="left",
            )  # Horizontally align to the left of the bar
    ...
    for file_path in file_paths:
        ...
        if os.path.exists(file_path):
            ...
            if quantized_w_sum > 0:
                label = f"{parent_dir}"  # Use the parent directory as part of the label
                print(
                    f"File: {label}, Sum of Unique Quantized B Values: {quantized_w_sum}"
                )
                quantized_w_sums.append(quantized_w_sum)
                file_names.append(label)
        else:
...
def plot_pareto_accuracy(accuracy_log_path, quantized_log_path, save_dir=None):
    ...
    with open(accuracy_log_path) as acc_file:
        accuracy = [
            float(line.strip()) for line in acc_file if line.strip()
        ]  # Ignore empty lines
        print(len(accuracy))

    # Step 2: Load and count unique values from unique_combined_RowWiseQuantized_w_b.log
    unique_values_per_epoch = {}
    with open(quantized_log_path) as quant_file:
    ...
    unique_values_per_epoch = [
        len(np.unique(values)) for key, values in unique_values_per_epoch.items()
    ]

    if len(accuracy) != len(unique_values_per_epoch):
        raise ValueError(
            "Mismatch between number of epochs in accuracy log and quantized values log."
        )
...
def plot_pareto_loss(loss_log_path, quantized_log_path, save_dir=None):
    ...
    with open(loss_log_path) as acc_file:
        loss = [
            float(line.strip()) for line in acc_file if line.strip()
        ]  # Ignore empty lines
        print(len(loss))
    ...
    if len(loss) != len(unique_values_per_epoch):
        raise ValueError(
            "Mismatch between number of epochs in loss log and quantized values log."
        )
    ...
    for i in range(len(loss)):
        plt.text(
            loss[i], unique_values_per_epoch[i] + 6, f"{i + 1}", fontsize=9, ha="right"
        )
...
def plot_pareto_losses(paths, skip_epochs=0, skip_last_epochs=0):
    ...
    for idx, path in enumerate(paths):
        ...
        pr = [
            loss_log_path.split("_")[i + 1]
            for i, item in enumerate(loss_log_path.split("_"))
            if loss_log_path.split("_")[i] == "pr"
        ][0]
        ...
        if skip_last_epochs > 0:
            ...
            unique_values_per_epoch = unique_values_per_epoch[
                skip_epochs:-skip_last_epochs
            ]
        else:
        ...
        plt.plot(
            ...
            "o-",
            color=colors(idx),
            label=f"Scenario with pr {pr}",
        )

    # Now plot lines connecting corresponding entries (same epoch) across different paths with different colors
    num_epochs = len(
        all_losses[0]
    )  # Assuming all paths have the same number of epochs after skipping
    for epoch_idx in range(num_epochs):
        epoch_losses = [
            all_losses[path_idx][epoch_idx] for path_idx in range(len(paths))
        ]
        epoch_unique_values = [
            all_unique_values[path_idx][epoch_idx] for path_idx in range(len(paths))
        ]
        ...
        plt.plot(
            ...
            alpha=0.5,
        )  # Dashed colored line to connect points

    plt.ylabel("Number of Unique Quantized Values")
    plt.xlabel("Loss")
    plt.title(
        "# of Unique Quantized Values vs Loss over Epochs for different penalty rates"
    )
...
def plot_pareto_accuracies(paths, skip_epochs=0, skip_last_epochs=0):
    ...
    plt.xlabel("Accuracy")
    plt.title(
        "# of Unique Quantized Values vs Accuracy over Epochs for different penalty rates"
    )
