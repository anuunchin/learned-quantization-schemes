...
class ScaleTrackingCallback(tf.keras.callbacks.Callback):
    def __init__(self, layer, log_dir):
        ...
        self.log_file_path_w_rounded = (
            f"{log_dir}/unique_{layer.__class__.__name__}_w_{layer.w.shape}.log"
        )
        self.log_file_path_b_rounded = (
            f"{log_dir}/unique_{layer.__class__.__name__}_b_{layer.b.shape}.log"
        )
        self.log_file_path_w_b_rounded = (
            f"{log_dir}/unique_combined_{layer.__class__.__name__}_w_b.log"
        )
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        w_quantized_rounded_unique = np.unique(
            tf.floor(self.layer.w / self.layer.scale_w).numpy()
        )
        ...
        b_quantized_rounded_unique = np.unique(
            tf.floor(self.layer.b / self.layer.scale_b).numpy()
        )
        ...
        combined_quantized = np.union1d(
            w_quantized_rounded_unique, b_quantized_rounded_unique
        )
...
class AccuracyLossTrackingCallBack(tf.keras.callbacks.Callback):
    def __init__(
        self, layer, log_dir, accuracy_file="accuracy.log", loss_file="loss.log"
    ):
