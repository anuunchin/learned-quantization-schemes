...
class SCCE:
    ...
    def compute_total_loss(self, y_true, y_pred):
        ...
        tf.print(
            ...
            output_stream=f"file://{self.log_dir}/total_loss_log.log",
        )
    ...
    def compute_scale_penalty(self):
        ...
        tf.print(
            tf.reduce_mean(scale_penalty),
            output_stream=f"file://{self.log_dir}/scale_loss_log.log",
        )
...
class SCCEInverse:
    ...
    def compute_scale_penalty(self):
        ...
        for layer_index in range(len(self.weight_scales)):
            ...
            mean_inverse_w = tf.reduce_mean(
                1.0 / layer_weight_scales
            )  # Resulint in layer_weight_scales
...
class SCCEMinMaxBin:
    ...
    def compute_total_loss(self, y_true, y_pred):
        ...
        tf.print(
            "Loss:",
            tf.reduce_mean(total_loss),
            output_stream="file://logs/total_loss_log.txt",
        )
    ...
    def compute_scale_penalty(self):
        ...
        for layer_index in range(len(self.weight_scales)):
            ...
            max_w_quantized = tf.floor(
                tf.reduce_max(
                    tf.abs(layer_weights / layer_weight_scales),
                    axis=self.application_of_scale_factors,
                )
            )
            min_w_quantized = tf.floor(
                tf.reduce_min(
                    tf.abs(layer_weights / layer_weight_scales),
                    axis=self.application_of_scale_factors,
                )
            ...
            max_b_quantized = tf.floor(
                tf.reduce_max(tf.abs(layer_biases / layer_bias_scales))
            )
            min_b_quantized = tf.floor(
                tf.reduce_min(tf.abs(layer_biases / layer_bias_scales))
            )
            ...
            average_range_w_bins = (
                tf.reduce_mean(range_of_quant_w_bins) * dim_w
                + tf.reduce_mean(range_of_quant_b_bins) * dim_b
            )
        ...
        tf.print(
            "Loss:",
            tf.reduce_mean(scale_penalty),
            output_stream="file://logs/scale_loss_log.txt",
        )
...
class SCCEMaxBin:
    ...
    def compute_l2_regularization(self):
        ...
        tf.print(
            ...
            output_stream=f"file://{self.log_dir}/l2_loss_log.log",
        )
    ...
    def compute_scale_penalty(self):
        ...
        for layer_index in range(len(self.weight_scales)):
            ...
            tf.print(
                tf.floor(tf.reduce_mean(bins_w)),
                output_stream=f"file://{self.log_dir}/bins_w.log",
            )
            tf.print(
                tf.floor(tf.reduce_mean(bins_b)),
                output_stream=f"file://{self.log_dir}/bins_b.log",
            )
            ...
            average_bins = (
                tf.reduce_mean(bins_w) * dim_w + tf.reduce_mean(bins_b) * dim_b
            ) / (dim_w + dim_b)
            tf.print(
                tf.floor(average_bins),
                output_stream=f"file://{self.log_dir}/bins_average.log",
            )
            ...
            penalty_contribution, scale_num_contribution = tf.cond(
                ...
                lambda: (
                    ...
                    tf.constant(0.0, dtype=tf.float32),
                ),  # Zero penalty and no contribution to scale_num
                lambda: (
                    average_bins,
                    dim_b + dim_w,
                ),  # Regular penalty and scale_num update
            )
...
class SCCEDifference:
    ...
    def compute_total_loss(self, y_true, y_pred):
        ...
        scale_penalty = self.compute_scale_penalty()
        bin_scale_penalty = self.compute_bin_scale_penalty()

        l2_loss = self.compute_l2_regularization()

        total_loss = (
            cross_entropy_loss + scale_penalty + l2_loss + bin_scale_penalty
        )
    ...
    def compute_scale_penalty(self):
        ...
        for layer_index in range(len(self.weight_scales)):
            ...
            w_quantized_scaled_back = (
                tf.floor(layer_weights / layer_weight_scales) * layer_weight_scales
            )
            b_quantized_scaled_back = (
                tf.floor(layer_biases / layer_bias_scales) * layer_bias_scales
            )
    ...
    def compute_bin_scale_penalty(self):
        """
        Computes the penalty based on the number of bins calculated from the max weights divided by the quantization factor.
        Effectively punishes large number of bins.
        """
        ...
        tf.print(
            tf.reduce_mean(scale_penalty),
            output_stream=f"file://{self.log_dir}/bin_scale_loss_log.log",
        )
