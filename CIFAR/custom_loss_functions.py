...
class SCCEMaxBinL2:
    def __init__(self, layers, penalty_rate, log_dir, l2_lambda=0.01):
        self.w = layers[0].W
        self.b = layers[0].b
        self.qw_layer = layers[0].nested_q_w_layer
        self.qb_layer = layers[0].nested_q_b_layer
        self.w_scale = layers[0].nested_q_w_layer.scale
        self.b_scale = layers[0].nested_q_b_layer.scale

        self.w_2 = layers[1].W
        self.b_2 = layers[1].b
        self.qw_layer_2 = layers[1].nested_q_w_layer
        self.qb_layer_2 = layers[1].nested_q_b_layer
        self.w_scale_2 = layers[1].nested_q_w_layer.scale
        self.b_scale_2 = layers[1].nested_q_b_layer.scale
        ...
        self.logs = [
            ...
            "scale_loss_log.log",
            "l2_loss_log.log",
            "scale_loss_log_2.log",
            "l2_loss_log_2.log",
        ]

        self.losses_dir = os.path.join(log_dir, 'losses')
        os.makedirs(self.losses_dir, exist_ok=True)

        setup_logger(self.losses_dir, self.logs)
    ...
    def compute_l2_regularization(self):
        ...
        l2_loss += tf.reduce_sum(tf.square(self.w))  
        l2_loss += tf.reduce_sum(tf.square(self.b))    

        l2_loss += tf.reduce_sum(tf.square(self.w_2))  
        l2_loss += tf.reduce_sum(tf.square(self.b_2))    

        l2_loss *= self.l2_lambda  

        tf.print(tf.reduce_mean(l2_loss), output_stream=f'file://{self.losses_dir}/l2_loss_log.log')
    ...
    def compute_scale_penalty(self):
        """
        Computes the scale penalty based on the max number of bins.
        """

        max_bins_w = tf.reduce_max(tf.abs(self.w / self.w_scale))
        max_bins_b = tf.reduce_max(tf.abs(self.b / self.b_scale))

        scale_loss = (max_bins_w * 784 + max_bins_b) / 785

        max_bins_w_2 = tf.reduce_max(tf.abs(self.w_2 / self.w_scale_2))
        max_bins_b_2 = tf.reduce_max(tf.abs(self.b_2 / self.b_scale_2))

        scale_loss_2 = (max_bins_w_2 * 128 + max_bins_b_2) / 129

        scale_loss *= self.penalty_rate * (scale_loss + scale_loss_2)

        tf.print(tf.reduce_mean(scale_loss), output_stream=f'file://{self.losses_dir}/scale_loss_log.log')
    ...
    def compute_total_loss(self, y_true, y_pred):
        ...
        scale_loss = self.compute_scale_penalty()

        total_loss = cross_entropy_loss + scale_loss

        tf.print(tf.reduce_mean(total_loss), output_stream=f'file://{self.losses_dir}/total_loss_log.log')
